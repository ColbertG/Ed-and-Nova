    ...
    void Update()
    {
        if (ActiveSpawnBarrier)
        {
            ...
            if (BottomSpawnSet)
            {
                ...
                Vector3 BottomSpawn = Camera.main.transform.position + new Vector3(0, 0, 1);
                clone = Instantiate(Barrier[LevelSpawn], BottomSpawn, Quaternion.Euler(0, 0, 0)) as GameObject;
            }
            if (LeftSpawnSet)
            {
                LeftSpawnSet = false;
                Vector3 LeftSpawn = Camera.main.transform.position + new Vector3(0, 0, 1);
                clone = Instantiate(Barrier[LevelSpawn], LeftSpawn, Quaternion.Euler(0, 0, -90f)) as GameObject;
            }
            if (RightSpawnSet)
            {
                RightSpawnSet = false;
                Vector3 RightSpawn = Camera.main.transform.position + new Vector3(3, 0, 1);
