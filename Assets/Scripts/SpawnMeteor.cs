...
public class SpawnMeteor : MonoBehaviour
{
    ...
    List<GameObject> MeteorCount = new List<GameObject>();
    int LevelSpawnMin = 0;
    int LevelSpawn = 1;
    int SpawnCount = 0;
    void Awake()
    ...
    public bool MeteorDone() 
    {
        bool done = true;
        for (int i = 0; i < MeteorCount.Count; i++) 
        {
            if (MeteorCount[i] != null) 
            {
                done = false; 
                break;
            }
        }
        if(done) MeteorCount = new List<GameObject>();
        return done;
    }
    public void SpawnLevel(int level, int minLevel = 0) 
    {
        ...
        if (LevelSpawn > Meteor.Count) LevelSpawn = Meteor.Count;
        if(LevelSpawn <= 0) LevelSpawn = 1;
    }
    public float FaceingMeteor() 
    {
    ...
    public void FaceingMeteor(int spot) 
    {
        if (spot == 0|| spot == 1 ) 
        {
            RightSpawnSet = true;
